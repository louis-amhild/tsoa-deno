...
export class HonoTemplateService extends TemplateService<HonoApiHandlerParameters, HonoValidationArgsParameters, HonoReturnHandlerParameters> {
  constructor(
    ...
    super(models, minimalSwaggerConfig);
  }
  ...
  async apiHandler(params: HonoApiHandlerParameters): Promise<any> {
    ...
    const controllerOutput: ResponseTypes | Promise<ResponseTypes> = this.buildPromise(methodName, controller, validatedArgs);

    try {
      let statusCode = successStatus;
      let headers: ReturnType<Controller['getHeaders']> = {};
      // Pack the result (which might or might not be a promise) in a promise to resolve it
      const data = await Promise.resolve(controllerOutput);
    ...
    const values = Object.keys(args).map((key) => {
      ...
      switch (param.in) {
        case 'request':
          return context.req;
        case 'query':
          return this.validationService.ValidateParam(param, context.req.query[name], name, fieldErrors, false);
        case 'queries':
          return this.validationService.ValidateParam(param, context.req.query, name, fieldErrors, false);
        case 'path':
          return this.validationService.ValidateParam(param, context.req.param(name), name, fieldErrors, false);
        case 'header':
          return this.validationService.ValidateParam(param, context.req.raw.headers.get(name) ?? undefined, name, fieldErrors, false);
        case 'body':
          return this.validationService.ValidateParam(param, bodyProps, name, fieldErrors, false);
        case 'body-prop':
          return this.validationService.ValidateParam(param, bodyProps[name], name, fieldErrors, true, 'body.');
        case 'request-prop':
          return this.validationService.ValidateParam(param, bodyProps, name, fieldErrors, false);
        case 'formData': {
          ...
          if (files.length > 0 || args[key].dataType === 'array' && args[key]?.array?.dataType === 'file') {
            return this.validationService.ValidateParam(param, context.files[name] || undefined, name, fieldErrors, false);
          } else {
            return this.validationService.ValidateParam(param, context.form.get(name), name, fieldErrors, false);
          }
        }
        case 'res':
