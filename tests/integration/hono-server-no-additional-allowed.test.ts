...
import { server } from './../fixtures/hono/serverNoAdditional.ts';
import { Gender, GenericRequest, ParameterTestModel, TestModel, ValidateMapStringToAny, ValidateMapStringToNumber, ValidateModel } from '../fixtures/testModel.ts';

type SuperTest<T> = SuperDeno;
const basePath = '/v1';

// While the purpose of this file (hono-server-no-additional-allowed.spec.ts) is to test the validation of POST bodies, we should have at least one GET test
// Hono Server (with noImplicitAdditionalProperties turned on)
...
Deno.test('should call out any additionalProperties', () => {
...
Deno.test('should call out any additionalProperties on Unions', () => {
  const data = {
    ...getFakeModel(),
...
Deno.test('should call out any additionalProperties for nested literals', () => {
  const data = Object.assign({}, getFakeModel(), {
    objLiteral: {
      extra: 123,
      nested: {
...
Deno.test('should respect additional props', () => {
...
Deno.test('should be okay if there are no additionalProperties', () => {
