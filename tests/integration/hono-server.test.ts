...
import { basename, dirname, extname } from 'https://deno.land/std@0.214.0/path/mod.ts';
...
Deno.test('can invoke middlewares installed in routes and paths', () => {
...
Deno.test('should custom header', () => {
  return verifyGetRequest(
    ...
    (_err, res) => {
      ...
      // OBS: Hono returns a comma separated string of cookies, not an array (this might or might not be an issue for some clients)
...
Deno.test({
  name: 'can post a file',
  sanitizeOps: true,
  sanitizeResources: true,
  fn: () => {
    const formData = { someFile: `@${path.join("..", "package.json")}` };
    return verifyFileUploadRequest(basePath + '/PostTest/File', formData, (_err, res) => {
      const packageJsonBuffer = readFileSync(path.join('..', 'package.json'));
...
Deno.test({
  name: 'can post a file without name',
  sanitizeOps: true,
  sanitizeResources: true,
  fn: () => {
    const formData = { aFile: `@${path.join("..", "package.json")}` };
...
Deno.test({
  name: 'cannot post a file with wrong attribute name',
  sanitizeOps: true,
  sanitizeResources: true,
  fn: () => {
    const formData = { wrongAttributeName: `@${path.join("..", "package.json")}` };
...
Deno.test({
  name: 'can post multiple files with other form fields',
  sanitizeOps: true,
  sanitizeResources: true,
  fn: () => {
    const formData = {
      a: 'b',
      c: 'd',
      someFiles: [`@${path.join("..", "package.json")}`, `@${path.join("..", "tsconfig.json")}`],
    };
    ...
    return verifyFileUploadRequest(basePath + '/PostTest/ManyFilesAndFormFields', formData, (_err, res) => {
      for (const file of res.body as File[]) {
        const packageJsonBuffer = readFileSync(path.join('..', file.originalname));
        // TODO: Fix types? - TSOA file expects a buffer, however Hono givens a base64 string
...
Deno.test({
  name: 'can post multiple files with different field',
  sanitizeOps: true,
  sanitizeResources: true,
  fn: () => {
    const formData = {
      file_a: `@${path.join("..", "package.json")}`,
      file_b: `@${path.join("..", "tsconfig.json")}`,
    };
...
function getFakeModel(): TestModel {
  ...
  return testModel as unknown as TestModel;
}
